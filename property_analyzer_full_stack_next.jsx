...
// Per-unit fields: name, sqft, rentMode, rate (interpreted by mode), misc/mo.
...
function irrMonthly(cf){
  try{
    ...
    if (!(min < 0 && max > 0)) return null; // needs sign change
...
function unitMonthlyRent(u){
  const sqft = Number(u.sqft)||0; const rate = Number(u.rate)||0; const mode = u.rentMode||"monthly";
...
function compute(inputs){
  ...
  const baseMisc = units.reduce((a,u)=> a + (Number(u.misc)||0), 0);

  const rentGrowth = (inputs.rentGrowthAnnualPct ?? 3)/100;
  const expenseInfl = (inputs.expenseInflationAnnualPct ?? 2.5)/100;
  const otherOpsPct = (inputs.otherOpsPctOfRent ?? 30)/100;
  const sellingPct = (inputs.sellingCostsPct ?? 6)/100;

  const totalCost = (inputs.purchasePrice||0) + (inputs.closingCosts||0) + (inputs.loanCosts||0) + (inputs.rehab||0) + (inputs.miscCosts||0);
  const loanAmount = (inputs.loanPct/100) * (inputs.purchasePrice||0);
  const termMonths = Math.max(1, (inputs.amortYears||0)*12);
  const horizonMonths = Math.max(1, (inputs.holdYears||1)*12);
  const amort = amortSchedule(loanAmount, inputs.interestRatePct||0, termMonths, horizonMonths);
  const monthlyPmt = amort.payment||0;

  const vacancy = (inputs.vacancyPct||0)/100; const cfs=[-(totalCost - loanAmount)];
  ...
  for(let m=1;m<=horizonMonths;m++){
    ...
    const pm = ((inputs.propMgmtPct||0)/100)*collected;
  ...
  const exitCap = (inputs.exitCapPct||0)/100;
            ...
            {units.map((u,i)=> {
              ...
              return (
                <tr key={i} className="border-t">
                  ...
                  <td className="py-2 pr-3 text-right"><input type="number" className="w-24 rounded-lg border px-2 py-1 text-right" value={u.sqft||0} onChange={e=>update(i,'sqft',Number(e.target.value)||0)} /></td>
                  ...
                  <td className="py-2 pr-3 text-right">
                    <input type="number" className="w-28 rounded-lg border px-2 py-1 text-right" value={u.rate||0} onChange={e=>update(i,'rate',Number(e.target.value)||0)} />
                  </td>
                  <td className="py-2 pr-3 text-right">{fmt$(monthly)}</td>
                  <td className="py-2 pr-3 text-right"><input type="number" className="w-28 rounded-lg border px-2 py-1 text-right" value={u.misc||0} onChange={e=>update(i,'misc',Number(e.target.value)||0)} /></td>
  ...
  const Field = ({k,label,suffix,step=1}) => (
    <label className="block mb-3">
      <div className="flex items-center justify-between">
        ...
        {typeof inputs[k] === 'number' && <Badge tone="slate">{inputs[k]}</Badge>}
      </div>
      <input type="number" step={step} className="mt-1 w-full rounded-xl border px-3 py-2 focus:ring-2 focus:ring-indigo-500"
        value={inputs[k]}
        onChange={e=> setInputs(prev=>({...prev, [k]: Number(e.target.value)||0})) } />
      {suffix && <div className="text-xs text-slate-400 mt-1">{suffix}</div>}
    </label>
  );

  const Metric = ({label, value, kind, tone}) => (
          ...
          <section className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Section title="Units & Rents" emoji="🏢" tone="emerald">
                <UnitsEditor units={inputs.units} setUnits={(u)=> setInputs(prev=>({...prev, units: u}))} />
              </Section>
